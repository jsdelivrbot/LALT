...
OK, so we have 1 script and 1 init. for the [scrollReveal](http://scrollrevealjs.org/) plugin, the test runs like so...
...
Here i've added another test for a script I want to load, when I have a table that is sorted by the [stupidtable](https://github.com/joequery/Stupid-Table-Plugin) plugin. The test looks to see if the `[data-sort]` attrib. is attached to a `<th>` element. If it is, then the script is loaded, if not, nothing is loaded.
